...
class ParticipateInThreadsTest extends TestCase
{
	...
	#vendor/bin/phpunit tests/Feature/ParticipateInThreadsTest.php

	/** @test */
	function unauthenticated_user_may_not_participate_in_forum_threads()
	{
		$this->withExceptionHandling()
			->post( route( 'replies.store', [ 'a-channel', 1 ] ), [] )
    ...
    function an_authenticated_user_may_participate_in_forum_threads()
    {
	    ...
	    $this->post( route( 'replies.store', [ $thread->channel->slug, $thread->slug ] ), $reply->toArray() );
	...
	function users_may_only_reply_a_maximum_of_once_per_minute()
	{
		...
		$this->json( 'post', route( 'replies.store', [ $thread->channel->slug, $thread->slug ] ), $reply->toArray() )
			->assertStatus( 429 );
	}
}
